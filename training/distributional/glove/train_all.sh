...
do
    ...
    bash ./compute_cooccurrence.sh ~/git/Embeddings/output/corpus/en_corpus_ngrams_extended ~/git/Embeddings/output/distributional/glove/ $window;

    # Train for each dimension
    for dim in "${dims[@]}"
    do
        echo "nohup bash ./train.sh ~/git/Embeddings/output/corpus/en_corpus_ngrams_extended ~/git/Embeddings/output/distributional/glove/ $window $dim &"
    done
    echo ""

    # Convert textual format to binary
    for dim in "${dims[@]}"
    do
        echo "nohup python ../../utils/convert_text_embeddings_to_binary.py ~/git/Embeddings/output/distributional/glove/${dim}d/win${window}/wv.txt &"
    done
    echo ""
done
