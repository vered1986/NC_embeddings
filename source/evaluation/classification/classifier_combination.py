...
def main():
    ...
    ap.add_argument('embedding_path', help='## separated list of word embedding paths to use for w1 and w2 embeddings')
    ...
    for emb_path in args.embedding_path.split('##'):
        is_compositional = 'distributional' not in emb_path
        emb_dim = int(re.match('^.*/([0-9]+)d/.*$', emb_path).group(1))
    
        logger.info(f'Loading the embeddings from {emb_path}, with dimension {emb_dim}, compositional? {is_compositional}')
        wv, index2word = load_text_embeddings(emb_path, emb_dim)
        word2index = {w: i for i, w in enumerate(index2word)}
        
        prefix = 'comp_' if is_compositional else ''
        ...
        train_features.append(np.vstack([wv[word2index[key], :] if key in vocab else empty for key in train_keys]))
        test_features.append(np.vstack([wv[word2index[key], :] if key in vocab else empty for key in test_keys]))
        val_features.append(np.vstack([wv[word2index[key], :] if key in vocab else empty for key in val_keys]))
        ...
        train_features_c.append(np.vstack([np.concatenate([wv[cst1, :] if cst1 > 0 else empty,
                                                           wv[cst2, :] if cst2 > 0 else empty])
                                for cst1, cst2 in train_c]))
        test_features_c.append(np.vstack([np.concatenate([wv[cst1, :] if cst1 > 0 else empty,
                                                           wv[cst2, :] if cst2 > 0 else empty])
                                for cst1, cst2 in test_c]))
        val_features_c.append(np.vstack([np.concatenate([wv[cst1, :] if cst1 > 0 else empty,
                                                           wv[cst2, :] if cst2 > 0 else empty])
                                for cst1, cst2 in val_c]))

    train_features = np.concatenate(train_features, axis=-1)
    test_features = np.concatenate(test_features, axis=-1)
    val_features = np.concatenate(val_features, axis=-1)
    train_features_c = np.concatenate(train_features_c, axis=-1)
    test_features_c = np.concatenate(test_features_c, axis=-1)
    val_features_c = np.concatenate(val_features_c, axis=-1)
